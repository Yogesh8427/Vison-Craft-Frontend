...
function Admin() {
  ...
  const [item, setItem] = useState({ brand: "", model: "", price: 0, Quantity: 0, size: 0, category: "" });
  useEffect(() => {
        ...
        <div className='container-sm  shadow-sm p-1 rounded'>
          <form className='d-flex'>
            ...
            <div className='w-50 border p-3 rounded'>
              ...
              <select className='w-100 mb-4 p-2 border rounded' name='category' onChange={onChangeHandel}>
                <option >---Select---</option>
                <option value={"men"}>men</option>
                <option value={"women"}>women</option>
                <option value={"kids"}>kids</option>
              </select>
              <button type="submit" className="btn btn-success float-right" onClick={e => getData(e)}>Add</button>
            </div>
          </form>
        </div>
      </div>
